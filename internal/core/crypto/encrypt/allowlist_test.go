...
// goListDeps runs `go list -deps <pkg>` pinned to GOOS=linux GOARCH=amd64 and
// returns the transitive dep list. Pinning the evaluation platform makes the
// gate produce the same dep set on every dev host as in CI (ubuntu-latest,
